...
To store a payload inside the `.data` section, one simply needs to define it outside any function as a global variable:
```c
#include <Windows.h>
#include <stdio.h>
...
To store it inside the `.rdata`, append the keyword `const` to make it read-only:
```c
#include <Windows.h>
#include <stdio.h>
